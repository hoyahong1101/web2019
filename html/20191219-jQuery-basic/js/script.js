...
//	$("#id_sel").css("background", "#aaffff").css("color", "#0000ff");  //체이닝기법 : 연결하여 적용시 활용
	...
	$(".my_sel").siblings().css("border-left", "8px solid #ff0000");  //q본인을 제외한 모든 형제들을 선택
	$(".my_sel").siblings(".last_brother").css("border-bottom", "8px solid #ffff00");  //형제 중에서 특정적인 요소만을 선택이 가능
	...
	//input 요소 중 type="text"인 입력상자가 갖고 있는 value 값을 가져오겠다는 의미
	console.log($val_in);
...
//		1. 자동으로 부여된 인덱스 번호의 0에 따라서 <li>Laptop</li> 접근 
	...
	var $repeat = "";  //데이터 형태가 정의되지 않은 초기값  => undefined 데이터
	for(i=0; i<4; i++){
	...
	//html() 메서드와는 달리 text() 메서드는 내부에 들어가는 데이터를 텍스트로만 인식하기 때문에 태그명을 포함하였다면 태그명 조차도 텍스트로 인식이 됨
